...
func (c *Config) loadMatcherDictionaries() error {
	...
	if err := ensureAndLoadAliasJSON(filepath.Join(dir, "title_aliases.json"), defaults.TitleAliases, &loaded.TitleAliases); err != nil {
		return err
	}
	if err := ensureAndLoadAliasJSON(filepath.Join(dir, "transliteration_aliases.json"), defaults.TransliterationAliases, &loaded.TransliterationAliases); err != nil {
		return err
	}
	...
	if err := ensureAndLoadStringListJSON(filepath.Join(dir, "strip_patterns.json"), defaults.StripPatterns, &loaded.StripPatterns); err != nil {
		return err
	}
	...
	c.Matching.StripPatterns = mergeStrings(c.Matching.StripPatterns, loaded.StripPatterns)
...
func ensureAndLoadAliasJSON(path string, defaultValue map[string][]string, target *map[string][]string) error {
	if err := createJSONFileIfMissing(path, defaultValue); err != nil {
		return err
	}
...
func defaultMatcherDictionaries() matcherDictionaryDefaults {
	return matcherDictionaryDefaults{
		...
		TitleAliases: map[string][]string{
			"Миньоны":                             {"Minions"},
			"Chi's Sweet Home Atarashii Ouchi TV": {"Chi's Sweet Home"},
			"Достать ножи Стеклянная луковица": {"Glass Onion: A Knives Out Mystery"},
			"Кролик Питер":                 {"Peter Rabbit"},
			"Кролик Питер 2":               {"Peter Rabbit 2: The Runaway"},
			"Три кота":                     {"Kid-E-Cats"},
			...
			"Dead Man's Chest":             {"Pirates of the Caribbean: Dead Man's Chest"},
			"At World's End":               {"Pirates of the Caribbean: At World's End"},
			"On Stranger Tides":            {"Pirates of the Caribbean: On Stranger Tides"},
			"Dead Men Tell No Tales":       {"Pirates of the Caribbean: Dead Men Tell No Tales"},
		},
		TransliterationAliases: map[string][]string{
			"Posledniy Ministr":      {"The Last Minister"},
			"Posledniy ministr":      {"The Last Minister"},
			"K sebe nezhno":          {"К себе нежно", "Love Yourself Tender"},
			"Skazka o care Saltane":  {"Сказка о царе Салтане", "The Tale of Tsar Saltan"},
			"Skazka o tsare Saltane": {"Сказка о царе Салтане", "The Tale of Tsar Saltan"},
			"Утопия":                 {"Utopia"},
		},
		AnimeKeywords: []string{
...
func mergeAliasMap(base, extra map[string][]string) map[string][]string {
