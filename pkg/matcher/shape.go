...
func AnalyzeTorrentShape(folder string, filenames []string, rdType string, hasSeasonMarkers bool) TorrentShape {
	cfg := DefaultConfig()
	return analyzeTorrentShape(folder, filenames, rdType, hasSeasonMarkers, cfg.CollectionKeywords, cfg.ForceMoviePatterns)
}
...
func analyzeTorrentShape(folder string, filenames []string, rdType string, hasSeasonMarkers bool, collectionKeywords []string, forceMoviePatterns []*regexp.Regexp) TorrentShape {
	names := filenames
	...
	for _, name := range names {
		if season, ok := shapeSeason(name); ok {
			showFiles++
			if season != "" {
	...
	if season, ok := shapeSeason(folder); ok {
	...
	distinctTitles := countDistinctTitles(names)
...
func shapeSeason(s string) (string, bool) {
	...
	return "", seasonEpisodeRE.MatchString(s)
}
